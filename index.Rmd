...
author: "*Created by : Panggah D. Putra*"
...
Classification of the charts in this website is based on the [Financial Times Visual Vocabulary](https://github.com/Financial-Times/chart-doctor/tree/main/visual-vocabulary){target="blank"} with addition from other resources.
...
To make the dot strip plot, we use *df_for_dotstrip*.
...
To make the cumulative curve, we use *df_for_cum_curve*.
...
We also can ordered dot strip plot by certain value. In this example, we ordered dot strip plot by max value of each variable.
...
In dot strip plot, it can be a problem when to many dots have the same value. To solve this, we can use stat dot plot. The stat dot plot easily show multiple dots having the same value.
...
Unlike density plot, stat density ridges plot shows density of each variable. It also can shows quantile value of each variable.
