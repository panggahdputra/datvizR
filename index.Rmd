...
Seluruh grafik pada tipek grafik-korelasi menggunakan `gapminder` dataset dengan beberapa proses manipulasi data. Column + Line Timeline, Connected Scatter Plot, Bubble, serta XY Heatmap pada tipe grafik-korelasi juga menggunakan `gapminder` dataset.
...
```{r df for diverging bar, message=FALSE}

diver_bar_df <- gapminder %>%
...
```{r df for diverging stacked bar, message=FALSE}

diver_stack_bar_df <- gapminder %>%
  filter(year == 2007) %>%
  mutate(median_lifeExp = median(lifeExp)) %>%
  group_by(continent) %>%
  summarize(below_med = sum(lifeExp < median_lifeExp)/n(),
            beyond_med = sum(lifeExp >= median_lifeExp)/n()) %>%
  pivot_longer(!continent, names_to = 'cluster', values_to = 'perc_country') %>%
  mutate(perc_country = if_else(cluster == 'below_med', -perc_country, perc_country))

```

```{r df for spine, message=FALSE}
...
```{r df for surplus/deficit filled line, message=FALSE}

filled_line_df <- gapminder %>%
  filter(continent == 'Asia') %>%
  group_by(year) %>%
  mutate(median_gdp = median(gdpPercap)) %>%
  mutate(gdp_gap = gdpPercap - median_gdp) %>%
  filter(country == 'Jordan')

```

Scatter Plot dan Scatter Plot with Regression Line pada tipe grafik-korelasi dibuat menggunakan `iris` Dataset.
...
Tiap tipe grafik dibuat dengan menggunakan palet warna berbeda. Untuk mempercantik tipe grafik-deviasi, palet warna disalin secara manua dari <span style='color: green;'>**Tiepolo Palette**</span> di [MetBrewer](https://github.com/BlakeRMills/MetBrewer#tieplolo){target='blank'}

Untuk mempercantik tipe grafik-korelasi, palet yang digunakan adalah `palette_corr`. Untuk mempersempit fokus latihan, kode hex disalin secara manual dari <span style='color: green;'>**Redon Palette**</span> di [MetBrewer](https://github.com/BlakeRMills/MetBrewer#redon){target='blank'}. Anda dapat mempelajari sendiri penggunaan palet warna pada [MetBrewer](https://github.com/BlakeRMills/MetBrewer){target='blank'} tanpa harus menyalin code hex secara manual.
...
Untuk mempercantik tipe grafik-peringkat, palet yang digunakan adalah palet `palette_ranking`. Kode hex disalin secara manual dari <span style='color: green;'>**Greek Palette**</span> di [MetBrewer](https://github.com/BlakeRMills/MetBrewer#greek){target='blank'}. 
...
Untuk mempercantik tipe grafik-distribusi, palet yang digunakan adalah palet `palette_distr`. Kode hex disalin secara manual dari <span style='color: green;'>**Isfahan1 Palette**</span> di [MetBrewer](https://github.com/BlakeRMills/MetBrewer#isfahan1){target='blank'}.
...
Tiap tipe grafik dibuat dengan menggunakan tema berbeda. Untuk mempercantik tipe grafik-deviasi, tema yang digunakan adalah tema `theme_dev` di bawah ini.

```{r theme for deviation}

theme_dev <- theme_minimal() +
  theme(
    plot.title = element_text(color = '#802417',
                              hjust = 0.5,
                              face = 'bold'),
    plot.subtitle = element_text(color = '#802417',
                                 hjust = 0.5),
    plot.caption = element_text(color = '#802417',
                                hjust = 1),
    axis.title = element_text(color = '#802417',
                              face = 'bold'),
    axis.title.y = element_blank(),
    axis.text = element_text(color = '#802417',
                             face = 'bold'),
    axis.text.x = element_blank(),
    legend.position = 'none',
    panel.background = element_rect(fill = '#fffffe',
                                    color = NA),
    plot.background = element_rect(fill = '#fffffe',
                                   color = 'lightblue')
  )
...
Untuk mempercantik tipe grafik-korelasi, tema yang digunakan adalah tema `theme_corr` di bawah ini.

```{r theme for correlation}
...
diverging_bar <- diver_bar_df %>%
  ggplot(aes(x = lifeExp_gap,
             y = reorder(country, lifeExp_gap),
             fill = lifeExp_gap)) +
  geom_col(fill = if_else(diver_bar_df$lifeExp_gap > 0,
                          '#2b5851',
                          '#ce9344')) +
...
```{r diverging bar print, echo=FALSE, out.width = '100%'}
diverging_bar
```
...
```{r diverging stacked bar}

diverging_stacked_bar <- diver_stack_bar_df %>%
  ggplot(aes(x = perc_country,
             y = continent,
             fill = cluster)) + 
  geom_col(fill = if_else(diver_stack_bar_df$perc_country > 0,
                          '#2b5851',
                          '#ce9344')) +
  geom_text(aes(label = scales::percent(abs(perc_country), 2),
                hjust = ifelse(perc_country > 0, 1.1, -.1)),
            size = 5,
            fontface = 'bold',
            color = 'white') +
  geom_textvline(xintercept = 0,
...
```{r diverging stacked bar print, echo=FALSE, out.width = '100%'}
...
spine <- spine_df %>%
  ...
  geom_col(fill = if_else(diver_stack_bar_df$perc_country > 0,
                          '#2b5851',
                          '#ce9344')) +
  geom_text(aes(label = abs(perc_country),
                hjust = ifelse(perc_country > 0, -.1, 1.1)),
            size = 4,
            fontface = 'bold',
            color = '#2b5851') +
...
```{r spine print, echo=FALSE, out.width = '100%'}
spine
```
...
```{r surplus deficit filled line}

surplus_deficit_filled_line <- filled_line_df %>%
  ggplot(aes(x = year,
             y = gdp_gap)) +
  geom_area(color ='#2b5851',
            size = 1.2,
            fill = '#ce9344') +
  ...
  theme_dev +
  theme(
    axis.title.y = element_text(color = '#802417',
                              face = 'bold',
                              angle = 90),
    axis.text.x = element_text(color = '#802417',
                             face = 'bold')
  ) +
...
```{r surplus deficit filled line print, echo=FALSE, out.width = '100%'}
surplus_deficit_filled_line
```
...
col_line_tl <- gapminder %>%
  ...
  geom_textline(aes(x = year,
                    y = lifeExp*20),
                label = 'Life Expextancy', size = 3, vjust = -0.5,
                linewidth = 1, linecolor = '#1e395f', linetype = 2, 
                color = '#1e395f') +
...
```{r connected   print, echo=FALSE, out.width = '100%'}
con_scatter_plot
```
