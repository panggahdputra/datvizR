...
Seluruh grafik pada tipek grafik-deviasi menggunakan `gapminder` dataset dengan beberapa proses manipulasi data. Column + Line Timeline, Connected Scatter Plot, Bubble, serta XY Heatmap pada tipe grafik-korelasi juga menggunakan `gapminder` dataset.
...
diverging_bar <- diverging_bar_df %>%
  ...
  geom_textvline(xintercept = 0,
                 ...
                 color = '#802417') +
  theme_dev +
  theme(axis.line.y = element_line(color = '#802417',
                                   size = .5)) +
...
surplus_deficit_filled_line_df <- gapminder %>%
  filter(continent == 'Asia') %>%
  group_by(year) %>%
...
surplus_deficit_filled_line <- surplus_deficit_filled_line_df %>%
  ggplot(aes(x = year,
             y = gdp_gap)) +
