...
set.seed(2021)
df_for_bumb = tibble(tim = c('A','B','C','D','E'),
                          game_1 = sample(1:5),
                          game_2 = sample(1:5),
                          game_3 = sample(1:5),
                          game_4 = sample(1:5),
...
ordered_bar <- data_vis %>%
  ...
  ggplot(aes(x = reorder(Variable, -number_of_data), y= number_of_data,
...
lollipop <- data_vis %>%
  ...
  geom_col(alpha = 1, width = .05, color = '#2f70a1', fill = '#2f70a1', show.legend = FALSE) +
  geom_point(alpha = 1, size = 15, color = '#2f70a1', show.legend = FALSE) +
  geom_point(alpha = .7, size = 25, color = '#2f70a1', show.legend = FALSE) +
  geom_point(alpha = .4, size = 35, color = '#2f70a1', show.legend = FALSE) +
  geom_point(alpha = .1, size = 45, color = '#2f70a1', show.legend = FALSE) +
  geom_text(aes(x= Variable, y = number_of_data, label = number_of_data),
            size=5, color = 'yellow') +
  xlab('Variable') +
  ylab('number of data') +
  labs(title = 'Lollipop',
...
bump <- df_for_bumb_fix %>%
  ...
  geom_text(data = df_for_bumb_fix %>% filter(game == min(game)),
            aes(x = game - .1, label = tim), size = 5, hjust = 1) +
  geom_text(data = df_for_bumb_fix %>% filter(game == max(game)),
